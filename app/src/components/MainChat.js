...
export default function MainChat() {
    ...
    useEffect(() => {
        if (activeChannel) {
            ...
            socketService.joinWorkspace(activeWorkspace.id);
            socketService.onMessage((body) => {
                let message = JSON.parse(body);
                let workspaceId = message.workspaceId;
                dispatch(setChannelMessages({ channelId:message.receiver, messages: message }))
    ...
    return (
        <div className="flex flex-col flex-1">
            ...
            <div className="bg-white p-4 shadow-md flex items-center">
                <h2 className="text-lg font-semibold text-gray-800">{(activeChannel)? activeChannel.name: ""}</h2>
            </div>

            {/* Chat Messages */}

            <div className="flex-1 overflow-y-auto p-6 space-y-2 bg-white">
                {(activeChannel && channelMessages[activeChannel.id] || []).map((msg, index) => (
                <div className="flex items-start space-x-3" key={index}>
                    <img className="w-8 h-8 rounded-full" src="https://via.placeholder.com/40" alt="User"/>
                    <div

                        className={`p-2 rounded-lg ${msg.sender === user.id ? "bg-blue-500 ml-auto" : "bg-gray-700"}`}
                    >
                        <h4 className="font-semibold text-gray-800 text-sm">
                            {msg.sender} <span className="text-xs text-gray-500 ml-2">{msg.when}</span>
                        </h4>
            ...
            <div className="p-4 bg-gray-100 border-t flex items-center">
                ...
                />
                <button className="ml-2 px-4 py-2 bg-blue-600 text-white rounded-lg" disabled={isButtonDisabled} onClick={() => sendMessage()}>Send</button>
            </div>
        </div>
    )
}