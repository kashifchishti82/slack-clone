...
export default function SidebarChannel() {
  ...
  useEffect(() => {
    if (data) {
      ...
      if (data.WorkspaceChannels.length > 0) {
        dispatch(setActiveChannel(data.WorkspaceChannels[0]));
      }
    }
  }, [data]);
      ...
      <ul className="mt-2 space-y-1">
        {channels &&
          channels.map((channel) => (
            <li
              className="py-2 px-3 bg-gray-700 rounded cursor-pointer"
              key={channel.id}
        ...
        <li
          className="py-2 px-3 hover:bg-gray-700 rounded cursor-pointer"
          key={"add-channel"}
          onClick={openModal}
        >
          Add Channel
